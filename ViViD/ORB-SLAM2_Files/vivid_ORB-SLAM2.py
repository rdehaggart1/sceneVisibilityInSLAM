...
def main(*args):
    ...
    # the ground truth file starts from 0.0
    ...
    # get the position data and extract x, y, z (use ViViD marker 1)
    ...
    # save the start time to subtract from the estimate time
    startTimeGT = df['header.stamp.secs'][0] + pow(10,-9) * df['header.stamp.nsecs'][0]
    # now store the timestamps in a list starting from 0.0
    timestampGT = list(df['header.stamp.secs'] + pow(10,-9) * df['header.stamp.nsecs'] - startTimeGT)
    poseGT[0] = [float(row[2]) for row in poseGraphGroundTruth] # x1
    poseGT[1] = [float(row[3]) for row in poseGraphGroundTruth] # y1
    poseGT[2] = [float(row[4]) for row in poseGraphGroundTruth] # z1
    ...
    # format: (0)time, (1)x, (2)y, (3)z, (4)qx, (5)qy, (6)qz, (7)qw
        ...
        # vivid coordinate frame agrees with this convention
    timestampEst = [float(row[0]) - startTimeGT for row in poseGraphEstimate]
    ...
    T_cam_gt = [[0.538117713753331,	0.134273164984585,	-0.832105788532870,	-21.1235087691594],
    ...
    xyz = np.array([[poseEst[0][a], poseEst[1][a], poseEst[2][a], 1] for a in range(len(poseEst[0]))])
    
    for i in range(len(xyz)):
        xyzTranslated = T_cam_gt @ xyz[i]
        poseEst[0][i] = xyzTranslated[0]
        poseEst[1][i] = xyzTranslated[1]
        poseEst[2][i] = xyzTranslated[2]
        
    toTransform = poseEst.copy()
    toTransform.append([1] * len(poseEst[0]))
    transformedEstimate = np.matmul(T_cam_gt, toTransform)
    ...
    SVE_t = [(float(row[0]) - startTimeGT) for row in SVE_timeSeries]
