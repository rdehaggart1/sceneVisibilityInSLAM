...
BRIEF:  This script self-contains the full process of running a segment of 
        the midair dataset on the V-SLAM algorithm, ORB-SLAM3. 
...
the ORB-SLAM3 process. This includes building the workspace, launching the 
...
def main(*args):
    ...
    shellFile = os.path.abspath(os.getcwd() + "/midair_ORB-SLAM3.sh")
    print("\n")
    ...
    # ORB-SLAM3 writes the pose estimate to a .txt file after completion
    # extract the timestamped pose estimate from the ORB-SLAM3 output file
...
def matchAxisConventions(camera, estimateDF): 
    # NOTE: x,y,z axes are different for MidAir and ORB-SLAM3, so the
